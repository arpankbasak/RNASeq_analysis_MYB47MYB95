...
load(file = paste(out.path, "deseq_objects.plotting.Rdata", sep = ""))

# Feed in the LFC table and LFC as input for pathview -- incase of thresholding out the non significant genes
idx <- str_detect(colnames(logFC_P), "^genotype") & str_detect(colnames(logFC_P), "logFC$")
logFC_P <- logFC_P[, idx]
...
setwd(paste(path,fig.path,"/KEGG_MAPs/genotype/", sep = ""))
