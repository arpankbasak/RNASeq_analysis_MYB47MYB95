# Script for MROAST differential geneset analysis summary for specific pathway
# @ARPAN KUMAR BASAK
...
load(paste(out.path, "deseq_objects.plotting.Rdata", sep = "")) #LFC data
edata.cpm <- read.table(file = paste(stat.path, "/log2cpm.deseq.txt", sep = ""), sep = "\t", header = TRUE) # ExpressionData # AnnotationData
load(file = paste(out.path, "GSET-GO_deseq_objects-treatment.plotting.Rdata", sep = "")) # GO-Data-mroast
...
edata.cpm.df$genotype <- factor(design$genotype[idx], levels = genotype$short)
levels(edata.cpm.df$genotype) <- genotype$name
edata.cpm.df$treatment <- factor(design$treatment[idx], levels = treatment$name)
    ...
    if(flag <= 1){
        ...
        ggplot(aes(x = genotype, y = norm_counts, group = treatment)) +
            ...
            labs(x = nam,
                 ...
                 colour = "", 
                 shape = "") +
            ggsave(paste(fig.path, "/GSET/GO_MROAST_genotype/", tmp,"_CPM_sig[",alpha,"]_ngenes[",length(go_gene_sig),"]_genotype.png", sep = ""), 
                   dpi = 300, 
                   device = "png", 
                   height = 16, 
                   width = 9, limitsize = F);
        ...
        ggplot(aes(x = treatment, y = logFC)) +
            ...
            labs(x = "",
                 ...
                 colour = "", 
                 shape = "") +
            ggsave(paste(fig.path, "/GSET/GO_MROAST_genotype/", tmp,"_LFC_sig[",alpha,"]_ngenes[",length(go_gene_sig),"]_genotype.png", sep = ""), 
                   dpi = 300, 
                   device = "png", 
                   height = 9, 
        ...
        df.tmp$go_id <- tmp
        df.tmp$lfc_path <- paste(path, fig.path, "/GSET/GO_MROAST_genotype/", tmp,"_LFC_sig[",alpha,"]_ngenes[",length(go_gene_sig),"]_genotype.png", sep = "")
        df.tmp$cpm_path <- paste(path, fig.path, "/GSET/GO_MROAST_genotype/", tmp,"_CPM_sig[",alpha,"]_ngenes[",length(go_gene_sig),"]_genotype.png", sep = "")
        df.tmp$gene_ids <- paste(go_gene_sig, collapse = ";")
        write.table(df.tmp, paste(out.path, "/gset_analysis/GO_MROAST_genotype/", tmp,"_LFC_sig[",alpha,"]_ngenes[",length(go_gene_sig),"]_genotype.png", sep = ""), sep = "\t")
