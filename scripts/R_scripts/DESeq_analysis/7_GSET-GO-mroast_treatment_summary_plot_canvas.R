    ...
    if(flag <= 1){
        ...
        ggplot(aes(x = genotype, y = norm_counts, group = treatment)) +
            ...
            labs(x = nam,
                 ...
                 colour = "", 
                 shape = "") +
            ggsave(paste(fig.path, "/GSET/GO_MROAST_treatment/", tmp,"_CPM_sig[",alpha,"]_ngenes[",length(go_gene_sig),"]_treatment.png", sep = ""), 
                   dpi = 300, 
                   device = "png", 
                   height = 16, 
                   width = 9, limitsize = F)
        ...
        ggplot(aes(x = genotype, y = logFC)) +
            ...
            labs(x = "",
                 ...
                 colour = "", 
                 shape = "") +
            ggsave(paste(fig.path, "/GSET/GO_MROAST_treatment/", tmp,"_LFC_sig[",alpha,"]_ngenes[",length(go_gene_sig),"]_treatment.png", sep = ""), 
                   dpi = 300, 
                   device = "png", 
                   height = 9, 
        ...
        df.tmp$go_id <- tmp
        df.tmp$lfc_path <- paste(path, fig.path, "/GSET/GO_MROAST_treatment/", tmp,"_LFC_sig[",alpha,"]_ngenes[",length(go_gene_sig),"]_treatment.png", sep = "")
        df.tmp$cpm_path <- paste(path, fig.path, "/GSET/GO_MROAST_treatment/", tmp,"_CPM_sig[",alpha,"]_ngenes[",length(go_gene_sig),"]_treatment.png", sep = "")
        df.tmp$gene_ids <- paste(go_gene_sig, collapse = ";")
        write.table(df.tmp, paste(out.path, "/gset_analysis/GO_MROAST_treatment/", tmp,"_ngenes[",length(go_gene_sig),"]_treatment_table.txt", sep = ""), sep = "\t")
