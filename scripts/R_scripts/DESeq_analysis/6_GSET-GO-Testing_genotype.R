...
pkgs <- c("tidyverse", "biocparallel", "DESeq2", "limma", "deseq")
...
write.table(mroast.GO.long, 
            paste(out.path, "mroast-deseq_GSET_long_genotype.txt", sep = ""), sep = "\t")
...
write.table(camera.GO.long, 
            paste(out.path, "camera-deseq_GSET_long_genotype.txt", sep = ""), sep = "\t")
                      
save(list = c("mroast.GO.long", "camera.GO.long"), 
     file = paste(out.path, "GSET-GO_deseq_objects-genotype.plotting.Rdata", sep = ""))
