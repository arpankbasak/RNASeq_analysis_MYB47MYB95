# Script for MROAST geneset analysis
# @ARPAN KUMAR BASAK
...
# Make wide data
idx <- str_detect(mroast.GO.long$contrast, "^genotype") 
...
df_fdr %>% ggplot(aes(x = treatment, y = go_id, fill = sig)) +
    ...
    ggsave(paste(fig.path, 
                 "/GSET-GO-mroast-FDR[", alpha,"]-deseq_genotype_GO-clustered_heatmap.png", 
                 sep = ""), 
           dpi = 300, 
           device = "png", 
           height = 30, 
...
df_fdr %>% mutate(go_id = factor(go_id, levels = gene_sorted_ja)) %>%
    ...
    labs(x = "",
         ...
         fill =paste("vs Col-0 (FDR < ",alpha,")", sep = "")) +
    ggsave(paste(fig.path, 
                 "/GSET-GO-mroast-FDR[", alpha,"]-deseq_genotype_GO-JA_sorted_heatmap.png", 
                 sep = ""), 
           dpi = 300, 
           device = "png", 
           height = 30, 
...
VennDiagram::venn.diagram(x = regulated,
                          ...
                          filename = paste(fig.path, "/DEGs-all_deseq_genotype_VenD.png", sep = ""),
                          imagetype = "png",
                          sigdigs = 2,
                          hyper.test = TRUE, 
                          lower.tail = TRUE,
...
VennDiagram::venn.diagram(x = upregulated,
                          ...
                          filename = paste(fig.path, "/DEGs-up_deseq_genotype_VenD.png", sep = ""),
                          imagetype = "png",
                          sigdigs = 2,
                          hyper.test = TRUE, 
                          lower.tail = TRUE,
...
VennDiagram::venn.diagram(x = downregulated,
                          ...
                          filename = paste(fig.path, "/DEGs-down_deseq_genotype_VenD.png", sep = ""),
                          imagetype = "png",
                          sigdigs = 2,
                          hyper.test = TRUE, 
                          lower.tail = TRUE,
...
save(list = c("regulated", "downregulated", "upregulated"),
     file = paste(out.path, "/GO-list-mroast_deseq-genotype.Rdata", sep = ""))
