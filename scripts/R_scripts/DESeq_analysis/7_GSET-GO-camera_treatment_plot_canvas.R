...
# Raad in data
load(file = paste(out.path, "GSET-GO_deseq_objects-treatment.plotting.Rdata", sep = ""))
...
df_fdr %>% ggplot(aes(x = genotype, y = go_id, fill = sig)) +
    ...
    ggsave(paste(fig.path, 
                 "/GSET/GSET-GO-camera-FDR[", alpha,"]-deseq_treatment_GO-clustered_heatmap.png", 
                 sep = ""), 
           dpi = 300, 
           device = "png", 
           height = 30, 
...
df_fdr %>% mutate(go_id = factor(go_id, levels = gene_sorted_wt)) %>%
    ...
    labs(x = "",
         ...
         fill =paste("vs DW Treatment (FDR",alpha,")", sep = "")) +
    ggsave(paste(fig.path, 
                 "/GSET/GSET-GO-camera-FDR[", alpha,"]-deseq_treatment_GO-WT_sorted_heatmap.png", 
                 sep = ""), 
           dpi = 300, 
           device = "png", 
           height = 30, 
...
VennDiagram::venn.diagram(x = regulated,
                          ...
                          filename = paste(fig.path, 
                                           "/GSET/GSET-GO-camera_all_treatment_VenD.png", 
                                           sep = ""),
                          imagetype = "png",
                          sigdigs = 2,
                          hyper.test = TRUE, 
...
VennDiagram::venn.diagram(x = upregulated,
                          category.names = label.idx,
                          filename = paste(fig.path, 
                                           "/GSET/GSET-GO-camera_up_treatment_VenD.png", 
                                           sep = ""),
                          imagetype = "png",
                          sigdigs = 2,
                          hyper.test = TRUE, 
...
VennDiagram::venn.diagram(x = downregulated,
                          category.names = label.idx,
                          filename = paste(fig.path, 
                                           "/GSET/GSET-GO-camera_down_treatment_VenD.png", 
                                           sep = ""),
                          imagetype = "png",
                          sigdigs = 2,
                          hyper.test = TRUE, 
...
save(list = c("regulated", "downregulated", "upregulated"),
     file = paste(out.path, "/GO-list-camera_deseq-treatment.Rdata", sep = ""))
