...
pkgs <- c("tidyverse", "biocparallel", "DESeq2", "limma")
...
load(paste(out.path, "gset_analysis/objects/edeger-biomart-summary_treatment.Rdata", sep = ""))
...
# Load GLM objects
load(paste(out.path, "deseq_objects.GLM.Rdata", sep = ""))

# Build indeces for the analysis
idx <- ids2indices(adata, row.names(de))
idy <- unlist(lapply(idx, length))
idy <- which(between(idy, 3, 200)) # Specify range of number of genes to be analysed
...
write.table(mroast.GO.long, 
            paste(out.path, "mroast-deseq_GSET_long_treatment.txt", sep = ""), sep = "\t")

message("DONE")                      

# Conduct CAMERA analysis
...
write.table(camera.GO.long, 
            paste(out.path, "camera-deseq_GSET_long_treatment.txt", sep = ""), sep = "\t")
                      
save(list = c("mroast.GO.long", "camera.GO.long"), 
     file = paste(out.path, "GSET-GO_deseq_objects-treatment.plotting.Rdata", sep = ""))
