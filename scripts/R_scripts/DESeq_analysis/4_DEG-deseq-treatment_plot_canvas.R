...
#     gather(key = "interaction", value = "vals", convert = FALSE, -geneids, -cluster_k) %>%
...
# lfc_annotated <- cbind.data.frame(logFC_P, logFC_P.annotated[, !str_detect(colnames(logFC_P.annotated), "^treatment")]) %>%
# mutate(
#   myb_JA_vs_wt_JA = treatment_myb_JA_logFC - treatment_WT_JA_logFC,
#   myc.tKO_JA_vs_wt_JA = treatment_myc.tKO_JA_logFC - treatment_WT_JA_logFC
# )
# row.names(lfc_annotated) <- row.names(logFC_P)
# write.table(lfc_annotated, "./statistics/annotated_WT_JA_normalised.txt", sep = "\t", row.names = TRUE,  quote = FALSE)
...
if(file.exists(paste(out.path, "/LFC-clusters_edger.Rdata", sep = ""))){

  load(paste(out.path, "/LFC-clusters_edger.Rdata", sep = ""))
  ...
  save(list = c("clust_gene_lfc", "gene_clusters_lfc"),
       file = paste(out.path, "/LFC-clusters_edger.Rdata", sep = ""))
}

## ER body list
# idx <- grepl((logFC_P.annotated$symbol), pattern = "MEB|NAI|PYK|BGLU", ignore.case = TRUE)
# temp <- logFC_P.annotated[idx,]
...
# idx <- grepl((logFC_P.annotated$gene_name), pattern = "oxime", ignore.case = TRUE)
# Differential Gene Data carpenting
idx <- str_detect(colnames(logFC_P), "^treatment")
colnames(logFC_P)[idx] <- str_replace_all(colnames(logFC_P)[idx], "treatment", "treatment_DW")
# idx <- str_detect(colnames(logFC_P), "^genotype_")
# colnames(logFC_P)[idx] <- str_replace_all(colnames(logFC_P)[idx], "genotype", "genotype_WT")


df_lfc <- logFC_P %>% 
            add_column(gene_id = row.names(logFC_P), .before = 1) %>%
            gather(key = "key", value = "val", convert = FALSE, -gene_id) %>%
            separate(key, into = c("hypothesis", "x1", "genotype", "treatment", "col"), 
                     convert = FALSE, sep = "_") %>%
            # group_by(gene_id, hypothesis, genotype) %>%
...
df_lfc$sig <- as.factor(ifelse(df_lfc$PValue < alpha & abs(df_lfc$logFC) > 1, 1 * sign(df_lfc$logFC), 0))
...
df_lfc <- cbind.data.frame(df_lfc, 
  logFC_P.annotated[idx, 
  !grepl(colnames(logFC_P.annotated), pattern = "logFC$|PValue$", ignore.case = TRUE)])
...
mutate(
  ...
  cluster_dens = log10(n_clusters$n_genes[match(as.character(.$cluster_k), n_clusters$cluster_k)])
    ...
    geom_text(aes(label = (cluster_k), y = cluster_dens), size = 2, fontface = "bold", vjust = 0.5, hjust = 0.5) +
...
(lfc_hmap <- df_lfc %>%
 mutate(gene_id = factor(gene_id, levels = gene_clusters_lfc),
     ...
     geom_raster(alpha = 1) +
     # geom_tile(fill = NA, width = 0.95, height = 0.95, aes(colour = sig)) +
     facet_grid(cluster_k ~ hypothesis + treatment, scale = "free", space = "free", switch = "y") +
     scale_fill_gradient2(low = gradient.low, 
                          mid = gradient.mid,
                          high = gradient.high, 
                          na.value = gradient.na) +
...
# Volcano Plot
lfc_cutoff <- c(-1, 1)
FDR_cutoff <- c(-log10(5e-2), -log10(5e-4), -log10(5e-6), -log10(5e-8))
...
mutate(gene_id = factor(gene_id, levels = gene_clusters_lfc),
    ...
    facet_grid(hypothesis ~ genotype, scale = "fixed", space = "free", switch = "y") +
