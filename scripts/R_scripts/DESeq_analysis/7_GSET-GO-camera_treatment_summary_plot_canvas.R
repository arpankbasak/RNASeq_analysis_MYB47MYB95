...
load(file = paste(out.path, "GSET-GO_deseq_objects-treatment.plotting.Rdata", sep = "")) # GO-Data-camera
...
idx <- str_detect(camera.GO.long$contrast, "^treatment") 
...
# Significant DEGs
idx <- str_detect(logFC_P.long$contrast, "^treatment")
...
edata.cpm.df$treatment <- factor(edata.cpm.df$treatment, levels = treatment$name)
    ...
    if(flag <= 1){
        ...
        ggplot(aes(x = genotype, y = norm_counts, group = treatment)) +
            ...
            labs(x = nam,
                 ...
                 colour = "", 
                 shape = "") +
            ggsave(paste(fig.path, "/GSET/GO_CAMERA_treatment/", tmp,"_CPM_sig[",alpha,"]_ngenes[",length(go_gene_sig),"]_treatment.png", sep = ""), 
                   dpi = 300, 
                   device = "png", 
                   height = 16, 
                   width = 9, limitsize = F);
        ...
        ggplot(aes(x = genotype, y = logFC)) +
            ...
            geom_boxplot(alpha = 0.4, outlier.fill = NA, aes(colour = genotype)) +
            geom_point(position = position_jitter(), 
                       alpha = 0.8,
                       aes(shape = gene_sig)) +
            scale_colour_manual(values = genotype$col.idx, labels = label.idx) +
            ...
            labs(x = "",
                 ...
                 colour = "", 
                 shape = "") +
            ggsave(paste(fig.path, "/GSET/GO_CAMERA_treatment/", tmp,"_LFC_sig[",alpha,"]_ngenes[",length(go_gene_sig),"]_treatment.png", sep = ""), 
                   dpi = 300, 
                   device = "png", 
                   height = 9, 
        ...
        df.tmp$go_id <- tmp
        df.tmp$lfc_path <- paste(path, fig.path, "/GSET/GO_CAMERA_treatment/", tmp,"_LFC_sig[",alpha,"]_ngenes[",length(go_gene_sig),"]_treatment.png", sep = "")
        df.tmp$cpm_path <- paste(path, fig.path, "/GSET/GO_CAMERA_treatment/", tmp,"_CPM_sig[",alpha,"]_ngenes[",length(go_gene_sig),"]_treatment.png", sep = "")
        df.tmp$gene_ids <- paste(go_gene_sig, collapse = ";")
        write.table(df.tmp, paste(out.path, "/gset_analysis/GO_CAMERA_treatment/",
                                  tmp,"_ngenes[",length(go_gene_sig),"]_treatment_table.txt", 
                                  sep = ""), sep = "\t")
    }else {
        message("Trying next ...")
    }
