# Script for plotting the GO and GSET analysis
# @Arpan Kumar Basak
# Cleanup
rm(list = ls())
...
# Raad in data
load(file = paste(out.path, "GSET-GO_deseq_objects-genotype.plotting.Rdata", sep = ""))
...
idx <- str_detect(colnames(go_p), "_JA$") 
idy <- order(go_p[,idx])
gene_sorted_ja <- row.names(go_p)[idy]

# Looking for genes that relate to pathway
df_path <- go_p %>% 
...
df_fdr %>% ggplot(aes(x = treatment, y = go_id, fill = sig)) +
    geom_tile(alpha = 1, width = 0.6) +
    facet_grid(definition ~ genotype, switch = "y", scales = "free", space = "free") +
    ...
    labs(x = "",
         y = "",
         fill = paste("vs Col-0 (FDR < ", alpha,")", sep = "")) +
    ggsave(paste(fig.path, 
                 "/GSET/GSET-GO-camera-FDR[", alpha,"]-deseq_genotype_GO-clustered_heatmap.png", 
                 sep = ""), 
           dpi = 300, 
           device = "png", 
           height = 30, 
...
df_fdr %>% mutate(go_id = factor(go_id, levels = gene_sorted_ja)) %>%
    ggplot(aes(x = treatment, y = go_id, fill = sig)) +
    geom_tile(alpha = 1, width = 0.6) +
    facet_grid(definition ~ genotype, switch = "y", scales = "free", space = "free") +
    scale_fill_manual(values = c(`0` = gradient.na,
                                 `1` = gradient.sig.high, 
    ...
    labs(x = "",
         ...
         fill =paste("vs Col-0 (FDR < ",alpha,")", sep = "")) +
    ggsave(paste(fig.path, 
                 "/GSET/GSET-GO-camera-FDR[", alpha,"]-deseq_genotype_GO-JA_sorted_heatmap.png", 
                 sep = ""), 
           dpi = 300, 
           device = "png", 
           height = 30, 
...
regulated <- list(
    myb_dw = row.names(df_DGO)[which(df_DGO[,str_detect(colnames(df_DGO), "myb_DW")] != 0)], 
    myc_dw = row.names(df_DGO)[which(df_DGO[,str_detect(colnames(df_DGO), "myc.tKO_DW")] != 0)], 
...
VennDiagram::venn.diagram(x = regulated,
                          ...
                          filename = paste(fig.path, "/GSET/GSET-GO-all_camera_deseq_genotype_VenD.png", sep = ""),
                          imagetype = "png",
                          sigdigs = 2,
                          hyper.test = TRUE, 
                          lower.tail = TRUE,
...
compared to Col-0 (FDR < ", alpha, ")", sep = ""), 
                          print.mode = "percent",
                          main.cex = 0.4

)
upregulated <- list(
    myb_dw = row.names(df_DGO)[which(df_DGO[,str_detect(colnames(df_DGO), "myb_DW")] == 1)], 
    myc_dw = row.names(df_DGO)[which(df_DGO[,str_detect(colnames(df_DGO), "myc.tKO_DW")] == 1)],
...
VennDiagram::venn.diagram(x = upregulated,
                          ...
                          filename = paste(fig.path, "/GSET/GSET-GO-up_camera_deseq_genotype_VenD.png", sep = ""),
                          imagetype = "png",
                          sigdigs = 2,
                          hyper.test = TRUE, 
                          lower.tail = TRUE,
...
VennDiagram::venn.diagram(x = downregulated,
                          ...
                          filename = paste(fig.path, "/GSET/GSET-GO-down_camera_deseq_genotype_VenD.png", sep = ""),
                          imagetype = "png",
                          sigdigs = 2,
                          hyper.test = TRUE, 
                          lower.tail = TRUE,
...
save(list = c("regulated", "downregulated", "upregulated"),
     file = paste(out.path, "/GO-list-camera_deseq-genotype.Rdata", sep = ""))
