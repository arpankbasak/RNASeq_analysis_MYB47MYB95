...
write.table(mroast.GO.long, 
            paste(out.path, "mroast-edger_GSET_long_treatment.txt", sep = ""), sep = "\t")
...
write.table(camera.GO.long, 
            paste(out.path, "camera-edger_GSET_long_treatment.txt", sep = ""), sep = "\t")
                      
save(list = c("mroast.GO.long", "camera.GO.long"), 
     file = paste(out.path, "GSET-GO_edger_objects-treatment.plotting.Rdata", sep = ""))
