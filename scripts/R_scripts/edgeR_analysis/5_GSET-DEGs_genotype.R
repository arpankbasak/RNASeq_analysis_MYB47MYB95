...
# Raad in data
load(paste(out.path, "/DEG-list_edger-genotype.Rdata", sep = ""))
message("Saving gene lists ...")
sink(paste(paste(out.path, "/gset_analysis/edger-upregulated_genotype_gene_list.txt", sep = "")))
upregulated
sink(paste(paste(out.path, "/gset_analysis/edger-downregulated_genotype_gene_list.txt", sep = "")))
downregulated
sink()
