...
df_fdr %>% ggplot(aes(x = genotype, y = go_id, fill = sig)) +
    ...
    ggsave(paste(fig.path, 
                 "/GSET/GSET-GO-camera-FDR[", alpha,"]-edger_treatment_GO-clustered_heatmap.png", 
                 sep = ""), 
           dpi = 300, 
           device = "png", 
           height = 30, 
...
df_fdr %>% mutate(go_id = factor(go_id, levels = gene_sorted_wt)) %>%
    ...
    labs(x = "",
         ...
         fill =paste("vs DW Treatment (FDR",alpha,")", sep = "")) +
    ggsave(paste(fig.path, 
                 "/GSET/GSET-GO-camera-FDR[", alpha,"]-edger_treatment_GO-WT_sorted_heatmap.png", 
                 sep = ""), 
           dpi = 300, 
           device = "png", 
           height = 30, 
...
save(list = c("regulated", "downregulated", "upregulated"),
     file = paste(out.path, "/GO-list-camera_edger-treatment.Rdata", sep = ""))
