# Scripts for plotting differential genes from EDGER output
# @Arpan Kumar Basak
...
# MYC profile
df_target %>% 
            ggplot(aes(y = genotype, x = logFC, colour = treatment)) +
            ggtitle("Targetted Expression Profile vs WT") +
            ...
            labs(x = "logFoldChange", y = "",
                colour = "Treatment") +
            ggsave(paste(fig.path, "/genewise/genotype_(MYCs_MYBs)_target.png", sep = ""), 
                   dpi = 600, 
                   device = "png", 
                   height = 10, 
...
df_lfc %>% ggplot(aes(x = treatment, y = gene_id, fill = saturate(logFC))) +
    ...
    ggsave(paste(fig.path, "/DEGs/LFC-edger_genotype_lfc-clustered_heatmap.png", sep = ""), 
           dpi = 300, 
           device = "png", 
           height = 30, 
...
df_lfc %>% ggplot(aes(x = treatment, y = gene_id, fill = sig)) +
    ...
    ggsave(paste(fig.path, "/DEGs/LFC-sig[", alpha,"]-edger_genotype_lfc-clustered_heatmap.png", sep = ""), 
           dpi = 300, 
           device = "png", 
           height = 30, 
           width = 10, limitsize = F)

df_lfc$high_sig <- ifelse(-log10(df_lfc$PValue) > 15, 
                           as.character(df_lfc$gene_id), 
                           NA)

# Plot Volcano plot for all combinations of contrast
...
VennDiagram::venn.diagram(x = regulated,
                          ...
                          filename = paste(fig.path, "/DEGs/DEGs-all_edgeR_genotype_VenD.png", sep = ""),
                          imagetype = "png",
                          sigdigs = 2,
                          hyper.test = TRUE, 
                          lower.tail = TRUE,
...
VennDiagram::venn.diagram(x = upregulated,
                          ...
                          filename = paste(fig.path, "/DEGs/DEGs-up_edgeR_genotype_VenD.png", sep = ""),
                          imagetype = "png",
                          sigdigs = 2,
                          hyper.test = TRUE, 
                          lower.tail = TRUE,
...
VennDiagram::venn.diagram(x = downregulated,
                          ...
                          filename = paste(fig.path, "/DEGs/DEGs-down_edgeR_genotype_VenD.png", sep = ""),
                          imagetype = "png",
                          sigdigs = 2,
                          hyper.test = TRUE, 
                          lower.tail = TRUE,
