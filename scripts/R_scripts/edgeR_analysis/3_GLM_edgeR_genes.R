rm(list = ls())
# NOTE::
# Explicitly using edgeR pipeline
options(warn = 1,
       mc.cores = 8)
...
colnames(model) <- str_replace(colnames(model), "groups|random", "")
...
contrast.mat <- makeContrasts(
    ...
    # Effect of treatment
    treatment_WT_JA = (WT_DW - WT_JA),
    treatment_myb_JA = (myb_DW - myb_JA),
    treatment_myc.tKO_JA = (myc.tKO_DW - myc.tKO_JA),
    
    # Effect of genotype
    genotype_WT_myb_JA = (WT_JA - myb_JA),
    genotype_WT_myb_DW = (WT_DW - myb_DW),
    genotype_WT_myc.tKO_JA = (WT_JA - myc.tKO_JA),
    genotype_WT_myc.tKO_DW = (WT_DW - myc.tKO_DW),
...
edata.norm <- edata/exp(rowMeans(log(edata)))
counts.norm <- edata/edata.length

# Estimate the effective library size
eff.lib <- calcNormFactors(counts.norm) * colSums(counts.norm)
edata.norm <- sweep(edata.norm, 2, eff.lib, "*")
edata.norm <- log(edata.norm)

# delist <- DGEList(counts = edata, group = groups)
