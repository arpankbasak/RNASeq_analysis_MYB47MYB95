...
# Raad in data
load(paste(out.path, "/DEG-list_edger-treatment.Rdata", sep = ""))
message("Saving gene lists ...")
sink(paste(paste(out.path, "/gset_analysis/edger-upregulated_treatment_gene_list.txt", sep = "")))
upregulated
sink(paste(paste(out.path, "/gset_analysis/edger-downregulated_treatment_gene_list.txt", sep = "")))
downregulated
sink()
sink()
...
save(list = "bm_res_upregulated", 
     file = paste(out.path, "gset_analysis/objects/edeger-biomart_treatment_up.Rdata", sep = ""))
rm(list = "upregulated")
...
save(list = "bm_res_downregulated", 
     file = paste(out.path, "gset_analysis/objects/edeger-biomart_treatment_down.Rdata", sep = ""))
