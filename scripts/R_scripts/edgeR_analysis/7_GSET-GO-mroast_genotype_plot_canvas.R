...
df_fdr %>% ggplot(aes(x = treatment, y = go_id, fill = sig)) +
    ...
    ggsave(paste(fig.path, 
                 "/GSET-GO-mroast-FDR[", alpha,"]-edger_genotype_GO-clustered_heatmap.png", 
                 sep = ""), 
           dpi = 300, 
           device = "png", 
           height = 30, 
...
df_fdr %>% mutate(go_id = factor(go_id, levels = gene_sorted_ja)) %>%
    ...
    labs(x = "",
         ...
         fill =paste("vs Col-0 (FDR < ",alpha,")", sep = "")) +
    ggsave(paste(fig.path, 
                 "/GSET-GO-mroast-FDR[", alpha,"]-edger_genotype_GO-JA_sorted_heatmap.png", 
                 sep = ""), 
           dpi = 300, 
           device = "png", 
           height = 30, 
...
save(list = c("regulated", "downregulated", "upregulated"),
     file = paste(out.path, "/GO-list-mroast_edger-genotype.Rdata", sep = ""))
