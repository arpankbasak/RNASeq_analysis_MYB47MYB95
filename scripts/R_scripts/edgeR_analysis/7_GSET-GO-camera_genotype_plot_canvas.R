...
# Raad in data
load(file = paste(out.path, "GSET-GO_edger_objects-genotype.plotting.Rdata", sep = ""))
...
df_fdr %>% ggplot(aes(x = treatment, y = go_id, fill = sig)) +
    ...
    ggsave(paste(fig.path, 
                 "/GSET/GSET-GO-camera-FDR[", alpha,"]-edger_genotype_GO-clustered_heatmap.png", 
                 sep = ""), 
           dpi = 300, 
           device = "png", 
           height = 30, 
...
df_fdr %>% mutate(go_id = factor(go_id, levels = gene_sorted_ja)) %>%
    ...
    labs(x = "",
         ...
         fill =paste("vs Col-0 (FDR < ",alpha,")", sep = "")) +
    ggsave(paste(fig.path, 
                 "/GSET/GSET-GO-camera-FDR[", alpha,"]-edger_genotype_GO-JA_sorted_heatmap.png", 
                 sep = ""), 
           dpi = 300, 
           device = "png", 
           height = 30, 
...
VennDiagram::venn.diagram(x = regulated,
                          ...
                          filename = paste(fig.path, "/GSET/GSET-GO-all_camera_edgeR_genotype_VenD.png", sep = ""),
                          imagetype = "png",
                          sigdigs = 2,
                          hyper.test = TRUE, 
                          lower.tail = TRUE,
...
VennDiagram::venn.diagram(x = upregulated,
                          ...
                          filename = paste(fig.path, "/GSET/GSET-GO-up_camera_edgeR_genotype_VenD.png", sep = ""),
                          imagetype = "png",
                          sigdigs = 2,
                          hyper.test = TRUE, 
                          lower.tail = TRUE,
...
VennDiagram::venn.diagram(x = downregulated,
                          ...
                          filename = paste(fig.path, "/GSET/GSET-GO-down_camera_edgeR_genotype_VenD.png", sep = ""),
                          imagetype = "png",
                          sigdigs = 2,
                          hyper.test = TRUE, 
                          lower.tail = TRUE,
...
save(list = c("regulated", "downregulated", "upregulated"),
     file = paste(out.path, "/GO-list-camera_edger-genotype.Rdata", sep = ""))
