...
edata.length.filtered <- edata.length[row.names(edata.filtered),]
...
row.names(design) <- design$sample_name
edata.filtered <- edata.filtered[,row.names(design) ]
...
design <- design[colnames(edata.filtered),]
...
edata.norm <- edata.length.filtered/exp(rowMeans(log(edata.length.filtered)))
counts.norm <- edata.filtered/edata.norm
